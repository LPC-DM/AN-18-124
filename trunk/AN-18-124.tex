...
% Do not edit by hand!
\RCS$Revision: 461612 $
\RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/notes/AN-18-124/trunk/AN-18-124.tex $
\RCS$Id: AN-18-124.tex 461612 2018-05-24 20:06:38Z alverson $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%  Title page %%%%%%%%%%%%%%%%%%%%%%%%
\cmsNoteHeader{AN-18-124} % This is over-written in the CMS environment: useful as preprint no. for export versions
% >> Title: please make sure that the non-TeX equivalent is in PDFTitle below
\title{Search for Inelastic Dark Matter in Events with One Lepton-Jet and Missing Transverse Momentum}
...
pdftitle={Search for Inelastic Dark Matter in Events with One Lepton-Jet and Missing Transverse Momentum},%
pdfsubject={CMS},%
...
\begin{verbatim}
> svn co --depth=empty svn+ssh://svn.cern.ch/reps/tdr2 myDir
> cd myDir
> svn update --depth=empty utils
> svn update --depth=infinity utils/trunk
> svn update --depth=empty [papers|notes]
> svn update --depth=infinity [papers|notes]/XXX-YY-NNN
...
the \texttt{svn copy} command. Please see the \texttt{svn} \href{http://svnbook.red-bean.com/en/1.6/svn-book.pdf}{manual} for details, particularly
the chapter on \href{http://svnbook.red-bean.com/en/1.6/svn-book.pdf#svn.branchmerge}{branching and tagging} and \href{http://svnbook.red-bean.com/en/1.6/svn-book.pdf#svn.forcvs}{svn for cvs users}. Please do not change the depth of the directory structure to the top-level \TeX\ file for your document.

\emph{Please make sure to configure your svn client:} edit \path{~/.subversion/config} so that it appropriately tags pdf and other commonly used file types.
\begin{verbatim}
[auto-props]
...
\verb|\mathrm| in formulas with Greek symbols, otherwise you will end up with formulae looking like the second one below.

\begin{gather}
\phi = \text{a Greek letter}\\
\mathrm{\phi} = \text{a mistake}
\end{gather}
...
but only uppercase in Math Bold (\verb|\mathbf|). Use either \verb|\boldsymbol| or \verb|\boldmath| outside the math delimiters (\$) (but inside braces) to get bold symbols. Compare:
 ...
 \begin{tabular}{ll}
\verb|$\Psi \alpha$|& $\Psi \alpha$ \\
\verb|$\mathbold{\Psi \alpha}$|& $\mathbold{\Psi \alpha}$ \\
 \verb|$\mathbf{\Psi \alpha}$|& $\mathbf{\Psi \alpha}$ \\
 %\verb|$\bm{\Psi \alpha}$|&$\bm{\Psi \alpha}$\\
 \verb|{\boldmath$\Psi \alpha$}|&{\boldmath$\Psi \alpha$}\\
...
% \verb|$\bm{\Psi \otimes \beta}$|& $\bm{\Psi \otimes \beta}$\\
  \verb|{\boldmath$\Psi \otimes \beta$}|& {\boldmath$\Psi \otimes \beta$}
\end{tabular}
...
\begin{figure*}[hbtp]\centering
 \includegraphics[width=0.49\textwidth]{CMS-bw-logo}
 \caption{Figures inserted using includegraphics.}
  \label{fig:ex1}\end{figure*}
\end{verbatim}

Only use lines inside the table where necessary to distinguish regions. External lines are only used on the top and bottom.
...
Please only use dimensions for scaling figures in terms of the page, typically, \verb|\textwidth|. Most figures should have a width of either 0.48 (one column) or 0.98 (full width) of the \verb|\textwidth|.

The \texttt{subfig} package is included and can be used for PASs and ANs (\textit{but not papers}) to generate (a), (b), \etc labels under the subfigures through the use of the \texttt{subfloat} command.


%This figure will be side-by-side in CMS mode and stacked for two-column journal layout. The labels track.
%If the figures are of equal width and fill the page across the full width (width= ~0.48\textwidth each) the use of \cmsFigWidth is not necessary.
\begin{figure}[hbtp]
  \centering
    \includegraphics[width=0.49\textwidth]{CMS-bw-logo}
    \includegraphics[width=0.49\textwidth]{CMScol}
...
References (\cite{CMS_AN_2006-027,CMS_NOTE_2006-106, Brandt:1997gi, Buchmuller:1986zs,PTDR2,CMS-PAS-JME-10-003,LEPewkfits,Bertram:2000br,Khachatryan:2010mp, RooStats,HEPdata,HEPDataCMSexample}) should use standard
...
The latest version of the acknowledgments should be included from \url{https://twiki.cern.ch/twiki/bin/viewauth/CMS/Internal/PubAcknow} as of the date of submission.

Do not alter the acknowledgements or add any special recognition, e.g., to a theorist who helped with a calculation, without first obtaining the approval of the Publications Committee chair. In general, an acknowledgment such as "We thank XXX for providing calculations for the XXX cross section" is allowed if and only if the calculation is not public. The acknowledgment should be added at the beginning of the acknowledgements section, before the standard CMS acknowledgments, in its own paragraph. Note, however, that
\begin{itemize}
  \item an acknowledgment for ``useful conversations" is not allowed;
  \item if a theory calculation is publicly available, an acknowledgment is not allowed;
  \item if help was received from the authors of a software package to run software that is publicly available, an acknowledgment is not allowed.
\end{itemize}
\end{acknowledgments}
...
% Feel free to delete the lines from here to the end and the pdefs.tex file once you start modifying the template. The example definitions are
% pulled in by the \input{ptdr-definitions} at the beginning for this table, but of course the real ones are in ptdr-definitions.cls and are invoked by the cms-tdr.cls file.
%
{
\small%
...
\clearpage
\section{Particle symbols: PENNAMES (see above for PENNAMES2)\label{pennames}}
...
Download the TeXLive 2017 installation, \url{http://mirror.ctan.org/systems/mac/mactex/MacTeX.mpkg.zip}, and install (if not already done). This is a relatively large installation.
...
\section{\TeX{} torture tests}
\newcommand{\ttbarx}{\ensuremath{{\cmsSymbolFace{t}\overline{\cmsSymbolFace{t}}}}\xspace} % t-tbar
\begin{table}[hbt]
\centering
\topcaption{Testing various features}
\begin{scotch}{ll}%using the special tabular environment designed for compatibility with the APS
\verb|\ttbar|&\ttbar\\
\verb|$R_\ttbar$|&$R_\ttbar$\\
\verb|$R_{\ttbar}$|&$R_{\ttbar}$\\
\verb|\textsf{test: $R_{\ttbar}$}|&\textsf{test: $R_{\ttbar}$}\\
\verb|\textsf{test: $R_\ttbarx$}|&\textsf{test: $R_\ttbarx$}\\
\verb|$\kappa^\mathrm{b}_L \kappa^{\PQb}_L$|&$\kappa^\mathrm{b}_L \kappa^{\PQb}_L$\\
\verb|$\Pp\Pa$|&$\Pp\Pp$\\
\verb|\Pp\Pa|&\Pp\Pp\\
\end{scotch}
\end{table}
\section{Table details (experts only)}
Following are some examples of different ways to deal with problems with tables. Please remember that any tables we provide will most likely be reset following the journal style. Although we should make our tables look good, the actual \TeX{} that produces the  tables should as simple as feasible.
\begin{table}[htb]
...
\end{verbatim}
\bgroup
\renewcommand*{\arraystretch}{1.4} % * implies argument will not include paragraph breaks--a good default: http://tex.stackexchange.com/questions/1050/whats-the-difference-between-newcommand-and-newcommand
\begin{tabular}{c*{3}{>{$}c<{$}}} % repeat the last column specification 3 times
\multicolumn{1}{c}{col. 1} & \multicolumn{1}{c}{col. 2} & \multicolumn{1}{c}{col. 3} & \multicolumn{1}{c}{col. 4}\\
\hline
row 1& \ttbar & 7^{+12}_{-5.2} & \frac{1}{3}\\
